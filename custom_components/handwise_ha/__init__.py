"""Handwise-HA integration for Handwise/Handwise MGC01 mowers."""
...
from .api import HandwiseCloudApiClient, HandwiseGenieApiError, HandwiseShadowApiClient
from .const import (
    ATTR_AUTO_ZONES,
...
def _all_coordinators(hass: HomeAssistant) -> list[HandwiseGenieDataUpdateCoordinator]:
    entries = hass.data.get(DOMAIN, {})
    coordinators: list[HandwiseGenieDataUpdateCoordinator] = []
...
) -> list[HandwiseGenieDataUpdateCoordinator]:
...
def _resolve_manual_zone_ids(
    coordinator: HandwiseGenieDataUpdateCoordinator, selectors: list[str | int]
) -> list[int]:
...
async def _async_register_services(hass: HomeAssistant) -> None:
    ...
    async def _handle_start_full_mow(service_call) -> None:
        ...
        if not targets:
            raise HandwiseGenieApiError("No target Handwise mower found")
...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    """Set up the Handwise MGC01 integration."""
    return True


async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Set up Handwise MGC01 from a config entry."""
    ...
    if not devices:
        raise ConfigEntryNotReady("No Handwise devices found for this account")
    ...
    for device in devices:
        ...
        try:
            ...
            if fallback_region:
                ...
                if not iot_endpoint and not fallback_region.startswith("cn"):
                    iot_endpoint = (
                        HandwiseShadowApiClient.build_default_iot_endpoint_for_region(
                            fallback_region
                        )
                    )
                elif iot_endpoint and not fallback_region.startswith("cn"):
                    endpoint_region = HandwiseShadowApiClient.guess_region_from_endpoint(
                        iot_endpoint
                    )
        ...
        shadow_client = HandwiseShadowApiClient(
            session=session,
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload Handwise MGC01 config entry."""
