"""Helpers for Handwise mowing parameter state."""
...
def coerce_enabled_value(value: object) -> bool:
    """Map Handwise integer/bool/string toggles to a Python bool."""
...
def raw_int_value(value: object) -> int | None:
    """Return an integer from common Handwise shadow payload shapes."""
    if isinstance(value, bool):
        return int(value)
    if isinstance(value, int):
        return value
...
def nest_visual_inspection_level_from_option(option: str) -> int:
    """Map a visual inspection level option back to the raw Handwise value."""
